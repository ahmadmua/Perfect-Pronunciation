...
// Protocol to notify delegate of playback completion
protocol PlaybackDelegate: AnyObject {
    func playbackDidFinishSuccessfully()
}

// Extend `Notification.Name` to define a custom notification
extension Notification.Name {
    static let playbackFinished = Notification.Name("playbackFinished")
}

// Class to handle audio playback functionality
class AudioPlayBackController: NSObject, ObservableObject, AVAudioPlayerDelegate {
    ...
    // A URL property to hold the audio file's location
    var fileURL: URL? {
        didSet {
